...
{{--                                </div>--}}
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="filter_group_id" id="filter_group_id" class="form-control form-control-sm @error('filter_group_id') is-invalid @enderror">
                                            @foreach($filters as $filterGroup)
                                                <option value="{{ $filterGroup->id }}" {{ old('filter_group_id',$category->filter_group_id) == $filterGroup->id ? 'selected' : '' }}>
                                                    {{ $filterGroup->name_en }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="col-md-6 col-12">
                                    <div class="form-group">
                                        ...
                                        @if($category->image)
                                            <img src="{{ asset( $category->image) }}" alt="Category Image" style="max-height: 80px; margin-top: 10px;">
                                        @endif
                                        @error('image')
                                        <span class="text-danger">{{ $message }}</span>
                                        @enderror
