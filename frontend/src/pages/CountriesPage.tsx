...
export function CountriesPage() {
  ...
  const openCreate = () => {
    ...
    setForm({ country_code: '', status: 'active', sort: '0', translations });
  };
  ...
  const openEdit = (row: CountryRow) => {
    ...
    setForm({ country_code: row.country_code, status: row.status, sort: String(row.sort ?? 0), translations });
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3">
        <h2 className="text-2xl font-semibold text-[#2f2b3d] dark:text-[#d7d8ea]">{t.countries}</h2>
                  ...
                  <tr key={row.id} className="border-t border-[#ececf3] dark:border-[#44485f]">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-2">
                        <Button size="icon" variant="secondary" title={t.cities} onClick={() => navigate(`/cities?country_id=${row.id}`)}>
