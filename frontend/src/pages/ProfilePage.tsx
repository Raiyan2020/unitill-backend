...
export function ProfilePage() {
  ...
  const fetchProfile = async () => {
    ...
    try {
      ...
      const split = (payload?.name || '').trim().split(' ');
  ...
  const fullName = useMemo(() => `${profileForm.firstName} ${profileForm.lastName}`.trim(), [profileForm.firstName, profileForm.lastName]);

  const updateProfile = async () => {
    setSavingProfile(true);
    try {
      await api.put('/admin/profile', { name: fullName, email: profileForm.email });
      await fetchProfile();
    } finally {
        ...
        <Card className="shadow-[0_10px_24px_rgba(47,43,61,0.08)] dark:shadow-[0_10px_24px_rgba(0,0,0,0.25)]">
          <CardContent className="space-y-6 p-5">
            <div className="flex flex-wrap items-center gap-4">
              <div className="flex h-20 w-20 items-center justify-center rounded-xl bg-[#7367f0]/15 text-[#7367f0]">
            ...
            {loading ? (
              ...
              <>
                ...
                <div className="flex items-center justify-between pt-2">
                  ...
                  <Button onClick={updateProfile} disabled={savingProfile || !fullName}>
                    {t.save}
                  </Button>
                </div>
              </>
