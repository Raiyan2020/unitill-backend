...
type LanguageRow = { id: number; code: string; name: string };
type TranslationValue = { title: string; subtitle: string; description: string };
type Row = {
  id: number;
  section: string | null;
  is_active: boolean;
...
export function LegalAffairsPage() {
  const { t } = useI18n();
  ...
  const [form, setForm] = useState({
    ...
    is_active: true,
    sort_order: '0',
    translations: {} as Record<string, TranslationValue>,
  });
  ...
  const buildEmptyTranslations = () => {
    ...
    languages.forEach((l) => {
      out[l.code] = { title: '', subtitle: '', description: '' };
    });
    return out;
  };
  ...
  const openEdit = (row: Row) => {
    const mapped = buildEmptyTranslations();
    languages.forEach((l) => {
      mapped[l.code] = row.translations?.[l.code] || { title: '', subtitle: '', description: '' };
    });
    setEditing(row);
    setForm({
      section: row.section || '',
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3">
        <h2 className="text-2xl font-semibold text-[#2f2b3d] dark:text-[#d7d8ea]">{t.legalAffairs}</h2>
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr key={row.id} className="border-t border-[#ececf3] dark:border-[#44485f]">
                    ...
                    <td className="px-4 py-3">{row.translations?.en?.title || row.translations?.ar?.title || '-'}</td>
          ...
          <div className="flex min-h-full items-start justify-center py-4 md:items-center md:py-8">
            ...
            <CardContent className="max-h-[72vh] space-y-3 overflow-y-auto p-4 md:max-h-[78vh] md:p-6">
              ...
              {languages.map((language) => (
                <div key={language.id} className="grid gap-2 rounded-xl border border-[#ececf3] p-3 dark:border-[#44485f]">
                  <p className="text-xs font-semibold uppercase text-[#8a8da8]">{language.code}</p>
                  <Input
                    placeholder={`Title (${language.code})`}
                    value={form.translations[language.code]?.title || ''}
                    onChange={(e) => setForm((s) => ({ ...s, translations: { ...s.translations, [language.code]: { ...(s.translations[language.code] || { title: '', subtitle: '', description: '' }), title: e.target.value } } }))}
                  />
                  <Input
                    placeholder={`Subtitle (${language.code})`}
                    value={form.translations[language.code]?.subtitle || ''}
                    onChange={(e) => setForm((s) => ({ ...s, translations: { ...s.translations, [language.code]: { ...(s.translations[language.code] || { title: '', subtitle: '', description: '' }), subtitle: e.target.value } } }))}
                  />
                  <textarea
                    placeholder={`Description (${language.code})`}
                    value={form.translations[language.code]?.description || ''}
                    onChange={(e) => setForm((s) => ({ ...s, translations: { ...s.translations, [language.code]: { ...(s.translations[language.code] || { title: '', subtitle: '', description: '' }), description: e.target.value } } }))}
                    className="min-h-[80px] rounded-xl border border-[#dbdbe8] bg-white px-3 py-2 text-sm dark:border-[#4a4f68] dark:bg-[#2f3349]"
                  />
                </div>
              ))}
